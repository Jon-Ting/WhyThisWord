...
async function main() {
  ...
  const result = {
    strongToLn: Object.fromEntries(Object.entries(strongToLn).map(([k, v]) => [k, Array.from(v)])),
    lnToStrong: Object.fromEntries(Object.entries(lnToStrong).map(([k, v]) => [k, Array.from(v)])),
    strongToLemma: strongToLemma,
  };

  console.log(
    `✅ Processed ${Object.keys(result.strongToLn).length} Strong's numbers and ${Object.keys(result.lnToStrong).length} LN domains.`,
  );
